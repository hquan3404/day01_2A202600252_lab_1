...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_resp, gpt4o_lat = call_openai(prompt=prompt)
    mini_resp, mini_lat = call_openai_mini(prompt=prompt)
    
    # Cost estimate = (len(response.split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-5.1"]
    gpt4o_cost_estimate = (len(gpt4o_resp.split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-5.1"]
    return {
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]  # last 3 turns (user + assistant)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = "Prompt | GPT-5.1 (orig GPT-4o) Response | GPT-5 (orig Mini) Response | GPT-5.1 Latency | GPT-5 Latency"
    lines = [header, "-" * 80]
    for r in results:
        p = r["prompt"][:40].replace("\n", " ")
        g_resp = r["gpt4o_response"][:40].replace("\n", " ")
        m_resp = r["mini_response"][:40].replace("\n", " ")
...
if __name__ == "__main__":
    ...
    #         # Gọi hàm với tham số temperature tĩnh để test
    #     response, latency = call_openai(prompt=test_prompt, temperature=temp)
    #     print(f"\nTemperature: {temp}")
    #     print(f"Time: {latency:.3f}s")
